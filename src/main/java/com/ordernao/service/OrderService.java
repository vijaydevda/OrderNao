...
public class OrderService {
	...
	try {
	    /*
	     * here we are fetching payment details form two tables 1) if
	     * orderstable throw exception then finally block will be executed
	     * which will fetch data from dailytransaction table & in case if
	     * dailytransaction table throw exception then we return empty
	     * bean(this will be a case when delivery boy have not done any
	     * order and no money will be provided to him) 2)if orderstable
	     * successfully return data without exception then finally block
	     * will be executed it will fetch data from dailytransaction table
	     * so there are two cases here either it will also return data
	     * without any exception or it will throw exception
	     */
		...
		logger.info("Exit at getDeliveryBoyPaymentDetails(Service):Success (inside finally block)");
	    } catch (Exception e2) {
		logger.info("Exception(indside finally block) :- ", e2);
	    }
	}
	return paymentBean;
	...
	try {
	    int totalMoneyProvided = dao.getTotalMoneyProvided();
	    int totalServiceChargeToCollect = dao.getTotalServiceCharge();
